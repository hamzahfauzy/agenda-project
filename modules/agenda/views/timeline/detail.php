<?php get_header() ?>
    <style>
    .timeline-panel{background: #FFF;border: 1px solid #eaeaea;max-width:720px;margin-left:auto;margin-right:auto;padding:20px;}
                ...
                <div class="col-md-12">
                    <div class="timeline-panel">
                        <div class="timeline-heading">
                            <a href="<?=routeTo('agenda/timeline/detail',['id'=>$post->id])?>">
                        ...
                        <div class="timeline-footer">
                            <a href="<?=routeTo('agenda/timeline/detail',['id'=>$post->id])?>" class="text-muted">
    ...
    function submitComment()
    {
        ...
        fetch('/agenda/timeline/submit-comment',{
            method:'POST',
            body:formData
        }).then(res => {
            if(res.ok)
    ...
    {
        var request = await fetch('/agenda/timeline/get-all-comments?post_id='+post_id)
        if(request.ok)
        {
            var response = await request.json()
            if(response.data.length && response.data[response.data.length-1].id != lastCommentId)
            {
